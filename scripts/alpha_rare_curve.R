...
for (depth in depths) {
  target_depth_col_idx <- grepl(paste0("depth-", depth, "_iter-"), colnames(data))
...
################## 分组统计绘图  ###################
plotgroupdata <- plotdata %>%
  group_by(Depth, Group) %>%
  summarize(meanvalue = mean(Value), se = sd(Value))
...
#使p2和p1的y轴坐标一致
p2 <- p2 + ylim(ggplot_build(p1)$layout$panel_scales_y[[1]]$
                  range$
                  range)
...
p <- p1 +
  p2 +
  plot_layout(heights = c(1, 1), widths = c(1, 1), ncol = 2, nrow = 1)
